...
public class SpreadSheet extends JFrame {
		...
		if (tok.length() >= 2 && tok.charAt(0) >= 'A' && tok.charAt(0) < (char) ('A' + maxCols)) {
			...
			if (row >= 0 && row < maxRows) {
				...
				if (cells[row][col].bottom)
					return null;
				// return cellsTF[row][col].getText();
				System.out.println("Raw Val: " + cells[row][col].rawValue);
				return cells[row][col].rawValue;
			}
		}
		return null;
	}
	...
	public String parseFormula(StringTokenizer tokens, int depth) throws NumberFormatException {
		if (tokens.hasMoreTokens()) {
			...
			while (tokens.hasMoreTokens()) {
				...
				System.out.println("tok1: " + tok + ", tok 3: " + tok3);
				if (tok3 == null)
					return null;
				if (tok2.equals("+")) {
					tok = add(tok, tok3);
	...
	public void evaluate(int r, int c, int depth) {
		...
		if (formula.length() > 0 && formula.charAt(0) == '=') {
			try {
				if (depth <= maxRows * maxCols) {
					...
					if (tokens.hasMoreTokens() && (tokens.nextToken().equals("="))) {
						...
						if (val != null) {
							if (val.length() > 8 && val.matches("-?\\d*.?\\d+")) {
								double vald = Double.parseDouble(val);
								if (vald - 1 < 0 || vald % 1 == 0) {
									cellsTF[r][c].setText(String.format("%.3g", vald));
								} else {
									cellsTF[r][c].setText(String.format("%.7g", vald));
								}
							} else {
								cellsTF[r][c].setText(val);
							}
							cells[r][c].rawValue = val;
			...
			if (formula.length() > 8 && formula.matches("-?\\d*.?\\d+")) {
				double vald = Double.parseDouble(formula);
				if (vald - 1 < 0 || vald % 1 == 0) {
					cellsTF[r][c].setText(String.format("%.3g", vald));
				} else {
					cellsTF[r][c].setText(String.format("%.7g", vald));
	...
	public void cellChanged(JTextField cell) {
		...
		if ((f.length() <= 0 || f.charAt(0) != '=') && !(f.length() > 8 && f.matches("-?\\d*.?\\d+"))) {
